#ifndef PIN_H
#define PIN_H

const int ihm_init = 25,
    ihm_tirette = 26,
    ihm_couleur = 27,
    ihm_strategy = 28;

const int neopixel = 57;

const int pinRobot = 2;

// TFT pinout
const int TFT_CS = 43,
    TFT_DC = 54,
    TFT_RST = 255,
    TFT_MOSI = 44,
    TFT_SCK = 46,
    TFT_MISO = 45;

//const int T_CS_PIN = 51, T_IRQ_PIN = 52;

// Stepper Pinout
const int stepper_en = 24;

const int dir_1 = 10, 
    dir_2 = 11, 
    dir_3 = 12 ;
const int step_1 = 5, 
    step_2 = 6, 
    step_3 = 9 ;

// Actuator Pinout (Connector 1)
const int pinServo01 = 23; //P2 - 8
const int pinServo02 = 21; //A4 - 6
const int pinServo03 = 19; //D1 - 4

const int pinServo04 = 17; //A2 - 2
const int pinServo05 = 16; //A1 - 1
const int pinServo06 = 18; //D2 - 3

const int pinServo07 = 56; //D3 - 9
const int pinServo08 = 20; //A3 - 5
const int pinServo09 = 22; //P1 - 7

// BallGrabber Pinout (Connector 2)
//  EnableRegulator 
const int pinEnaTraco   = 39;   //D1 - 5
//  MotorDriver 
const int pinTurbine    = 37;   //P1 - 2
const int pinMotor      = 38;   //P2 - 1
//  ServoMotors
const int pinServoTrap      = 55; //D2 - 6
const int pinServoExtended  = 53; //D3 - 9

//  IHM BallGrabber
const int pinButtonGrabber01 = 34; //A1 - 8 - UP
const int pinButtonGrabber02 = 15; //A2 - 7 - DOWN
const int pinButtonGrabber03 = 14; //A3 - 4 - I
const int pinButtonGrabber04 = 33; //A4 - 3 - II



#endif // PIN_H